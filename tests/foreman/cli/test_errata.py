...
class TestHostCollectionErrataInstallTest:
    """CLI Tests for the errata management feature"""


@pytest.fixture(scope='module')
def rh_repo(module_org, module_lce, module_cv, module_activation_key):
    # add a subscription to the Satellite Tools repo to activation key
    setup_org_for_a_rh_repo(
        {
            ...
            'activationkey-id': module_activation_key.id,
        },
...
@pytest.fixture(scope='module')
def custom_repo(module_org, module_lce, module_cv, module_activation_key):
...
@pytest.fixture(scope='module')
def setUp(module_org, module_activation_key):
    ...
    for _ in range(VIRTUAL_MACHINES_COUNT):
        # create VM
        ...
        virtual_machine.register_contenthost(
            module_org['name'], module_activation_key=module_activation_key['name']
        )
    ...
    for virtual_machine in virtual_machines:
        virtual_machine.run(f'yum install -y {CUSTOM_PACKAGE}')
        result = virtual_machine.run(f'rpm -q {CUSTOM_PACKAGE}')
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_id_and_org_id(module_org, setup):
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_id_and_org_name(module_org):
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_id_and_org_label(module_org):
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_name_and_org_id(module_org):
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_name_and_org_name(module_org):
    ...
    @pytest.mark.tier3
    def test_positive_install_by_hc_name_and_org_label(module_org):
    ...
    def test_negative_install_by_hc_id_without_errata_info(module_org):
        ...
        assert "Error: Option '--errata' is required" in context.exception.stderr

    @pytest.mark.tier3
    def test_negative_install_by_hc_name_without_errata_info(module_org):
    ...
    @pytest.mark.tier3
    def test_negative_install_without_hc_info(module_org):
    ...
    def test_negative_install_by_hc_id_without_org_info(module_org):
        ...
        with pytest.raises(CLIReturnCodeError) as context:
            HostCollection.erratum_install(
                {'id': host_collection['id'], 'errata': [CUSTOM_ERRATA_ID]}
            )
        assert 'Error: Could not find organization' in context.exception.stderr
    ...
    @pytest.mark.tier3
    @pytest.mark.upgrade
    def test_positive_list_affected_chosts(module_org):
    ...
    @pytest.mark.tier3
    def test_install_errata_to_one_host(module_org):
        ...
        result = virtual_machines[0].run(f'yum erase -y {CUSTOM_PACKAGE}')
        ...
        result = virtual_machines[0].run(f'rpm -q {FAKE_1_CUSTOM_PACKAGE_NAME}')
        assert result.return_code == 1, "Unwanted custom package found."
        # Assert second host does have FAKE_2_CUSTOM_PACKAGE
        result = virtual_machines[1].run(f'rpm -q {FAKE_2_CUSTOM_PACKAGE}')
    ...
    def test_positive_list_affected_chosts_by_erratum_restrict_flag(
        module_org, module_cv, module_lce
    ):
        ...
        assert (
            CUSTOM_ERRATA_ID in erratum_id_list
    ...
    @pytest.mark.tier3
    def test_host_errata_search_commands(module_org, module_cv, module_lce):
        ...
        result = [item['name'] for item in result]
        assert virtual_machines[0].hostname in result
        assert virtual_machines[1].hostname not in result
        ...
        result = [item['name'] for item in result]
        assert virtual_machines[0].hostname not in result
        assert virtual_machines[1].hostname in result
...
@pytest.fixture(scope='class')
def org3():
    """an org with two custom products with repository"""
    # product small
...
@pytest.mark.tier3
def test_positive_list_filter_by_org_id_and_sort_by_updated_date(module_org, rh_repo, custom_repo):
...
@pytest.mark.tier3
def test_positive_list_filter_by_org_name_and_sort_by_updated_date(module_org, rh_repo, custom_repo):
...
@pytest.mark.tier3
def test_positive_list_filter_by_org_label_and_sort_by_updated_date(module_org, rh_repo, custom_repo):
...
@pytest.mark.tier3
def test_positive_list_filter_by_org_name_and_sort_by_issued_date(module_org, rh_repo, custom_repo):
...
@pytest.mark.tier3
def test_positive_list_filter_by_org_label_and_sort_by_issued_date(module_org, rh_repo, custom_repo):
...
def test_positive_list_filter_by_product_id(
    ...
    """
    org1.product_erratum_list = Erratum.list(
        {'product-id': org1.product.id, 'per-page': 1000}
    )
    org1.product_errata_ids = {
        errata['errata-id'] for errata in org1.product_erratum_list
    }
    org2.product_erratum_list = Erratum.list(
        {'product-id': org2.product.id, 'per-page': 1000}
    )
    org2.product_errata_ids = {
        errata['errata-id'] for errata in org2.product_erratum_list
    }
...
def test_positive_list_filter_by_product_id_and_org_name(
    ...
    def test_positive_list_filter_by_org_id(
        ...
        """
        org1_erratum_list = Erratum.list(
            {'organization-id': org1.id, 'per-page': 1000}
        )
        org1_errata_ids = {errata['errata-id'] for errata in org1_erratum_list}
        org2_erratum_list = Erratum.list(
            {'organization-id': org2.id, 'per-page': 1000}
        )
    ...
    def test_positive_list_filter_by_org_name(
        ...
        """
        org1_erratum_list = Erratum.list(
            {'organization': org1.name, 'per-page': 1000}
        )
        org1_errata_ids = {errata['errata-id'] for errata in org1_erratum_list}
        org2_erratum_list = Erratum.list(
            {'organization': org2.name, 'per-page': 1000}
        )
        org2_errata_ids = {errata['errata-id'] for errata in org2_erratum_list}
        assert len(org1_errata_ids) == ORG1_YUM_ERRATUM_COUNT
        assert len(org2_errata_ids) == ORG2_YUM_ERRATUM_COUNT
    ...
    def test_positive_list_filter_by_org_label(
        ...
        """
        org1_erratum_list = Erratum.list(
            {'organization-label': org1.label, 'per-page': 1000}
        )
        org1_errata_ids = {errata['errata-id'] for errata in org1_erratum_list}
        org2_erratum_list = Erratum.list(
            {'organization-label': org2.label, 'per-page': 1000}
        )
        org2_errata_ids = {errata['errata-id'] for errata in org2_erratum_list}
        assert len(org1_errata_ids) == ORG1_YUM_ERRATUM_COUNT
        assert len(org2_errata_ids) == ORG2_YUM_ERRATUM_COUNT
    ...
    def test_positive_user_permission(module_org):
        ...
        org_erratum_count = (
            org3.product_small_erratum_count + org3.product_big_erratum_count
        )
